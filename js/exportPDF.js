...
//						);
			...
			function exportPdfDoc(mymap, images){
				...
				mymap.fire('startprint', { camp: $('#itemTitle').text() });

				var ARABIC_REG = XRegExp('^.*\\p{Arabic}.*$');
...
//				doc.addImage(logo_cccm, 'PNG', 10, 10, 71.4375, 22.4896);
				...
				function addTextToDoc(text, maxWidthMm, x, vSpace, xMargin, doc){
					...
					if(ARABIC_REG.test(text)) {
						text = XRegExp.replace(text, XRegExp('\\p{Arabic}'), '', 'all');
					}

					text = text.replace(/'  '/g, ' ');
					text = text.replace(/ $/, '');

					doc.text(text, (x + xMargin), y, { align: 'left', maxWidth: maxWidthMm });	
					y += (doc.splitTextToSize(text, maxWidthMm).length * vSpace);
...
//									var s = t.lastIndexOf(/[^a-zA-Z\d]/g);
						...
						$.each(array, function(i, v){
							if(i < 1){
								// console.log('doc 0');
								// doc.text(x + xMargin, y, v, { align: "justify", maxWidth: 60 });
								doc.text(v, x + xMargin, y, { align: 'justify', maxWidth: 60 }, 0, 'justify');
								y += vSpace;
							} else {
								// console.log('doc 1');
								// doc.text(x, y, v, { align: "justify", maxWidth: 60 });
								doc.text(v, x, y, { align: 'justify', maxWidth: 60 }, 0, 'justify');
								y += vSpace;								
							}
						});
					} else {
						// console.log('doc 2');
						// doc.text(x + xMargin, y, text, { align: "justify", maxWidth: 60 });
						doc.text(text, x + xMargin, y, { align: 'justify', maxWidth: 60 }, 0, 'justify');
						y += vSpace;
					}
					*/
				}
				...
				function pageDef(){
					if (y > 282){
						if (col === 1){
							...
							y = 10;
							col = 2;
							doc.line(69, 10, 69, 287);
						} else if (col === 2){
							x += 60 + 7;
							y = 10;
							doc.line(137, 10, 137, 287);
							col = 3;
						} else if (col === 3){
							doc.addPage();
			        doc.setLineWidth(0.1);
				...
				//var mapHeightInPDF = 50 ; // size of map on paper (or set height but not both)
				var mapHeightInPDF = 37 ; // size of map on paper (or set height but not both)
				var mapPosXInPDF = 8; // pdf unit, mm
				var mapPosYInPDF = null; // pdf unit, mm

				var title = $('#itemTitle').text();

				if(XRegExp('^.*/.*\\p{Arabic}.*$').test(title)) {
					title = title.split('/')[0]
				}
        
        doc.setFontSize(10).setFontStyle('bold').setTextColor(0,0,0);											

        doc.text(title, x, y, { align:'left', maxWidth: 54 });
        y += (doc.splitTextToSize(title, 54).length * 5);
        ...
        y += 45;
				...
				$.each($('.categoryTitle'), function(i, v){					
					...
					addTextToDoc($(v).text(), 54, x + 5, 4, 0, doc);
	...
	//							doc.setFillColor(55,55,55,1);
								...
								addTextToDoc($(v).text(), 54, x, 2.5, 2.8, doc);
							} else {
								addTextToDoc($(v).text(), 54, x, 2.8, 0, doc);
							}
							...
							$.each(val, function(j, w){						
								addTextToDoc($(w).text(), 54, x + 5, 2.8, 0, doc);
							});
							...
							var hMm = (hPx/1.8) * 0.2578796561604585;
							var wPx = $("#" + v.id).width();
							// Pixel (Px) coversion to milimeters (Mm) ---- TODO : SHOULD BE RE-DONE CONSIDERING #pxMmTest
							var wMm = (wPx/1.8) * 0.2578796561604585;
